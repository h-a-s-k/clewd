...
const {randomInt: r, randomBytes: M} = require('node:crypto'), {version: G} = require('../package.json'), K = (new TextDecoder, 
new TextEncoder), U = 'clewd v' + G, Y = {
...
module.exports = {
    Main: U,
    AI: Y,
    encodeDataJSON: e => K.encode(`event: completion\ndata: ${JSON.stringify(e)}\n\n`),
    ...
    checkResErr: async (e, t = true) => {
        ...
        if (e.status && (e.status < 200 || e.status >= 300)) {
            ...
            s = Error(`${e.status}! ${e.statusText ? e.statusText + ', u' : 'U'}nexpected response code`);
        }
        if (r) {
            s.status = e.status;
            s.planned = true;
            r.message && r.message.startsWith('{') && r.message.endsWith('}') && (r.message = JSON.parse(r.message));
            if (r.message?.type || r.type) {
                s.type = r.message?.type || r.type;
                s.message = `${s.status}! ${s.type}`;
            }
            if (429 === e.status) {
                let e;
    ...
    indexOfAny: (e, t = false) => {
        let s = -1;
        const r = [ Z(e, t), Q(e, t) ].filter((e => e > -1)).sort();
        s = t ? r.reverse()[0] : r[0];
        return isNaN(s) ? -1 : s;
    },
    parseEvent: e => {
